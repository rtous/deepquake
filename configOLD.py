...
class Config(object):
  def __init__(self, config_file_path):
    setValues(self, "config_default.ini")
    setValues(self, config_file_path)

  def setValues(self, config_file_path):
    config = configparser.ConfigParser()
    config.read(config_file_path)
    
    if config.getboolean('main', 'debug') != None:
    ...
    if self.n_traces == 0:
        ...
        sys.exit(0)
    self.mean_velocity = config.getfloat('main', 'mean_velocity')
...
def getboolean(attribute, required):
    if config.getboolean('main', attribute) != None:
        setattr(self, attribute, config.getboolean('main', attribute))
    elif required:
        print("ERROR: Attribute "+attribute+" not found in default config file.")
        sys.exit(0)

def getint(attribute, required):
    if config.getint('main', attribute) != None:
        setattr(self, attribute, config.getint('main', attribute))
    elif required:
        print("ERROR: Attribute "+attribute+" not found in default config file.")
        sys.exit(0)

def getfloat(attribute, required):
    if config.getfloat('main', attribute) != None:
        setattr(self, attribute, config.getfloat('main', attribute))
    elif required:
        print("ERROR: Attribute "+attribute+" not found in default config file.")
        sys.exit(0)

def get(attribute, required):
    if config.get('main', attribute) != None:
